    ...
    <div class="result" v-if="isFound">
        ...
        <div v-for="(step, i) in recipeOutput.steps" :key="step.id">
            Step{{i+1}}: 
                ...
                <span class="answer" v-else>Préparation</span>
            <br>
...
export default {
    ...
    methods: {
        searchData(){
            let result = this.recipeInput.filter(recipe => recipe.id == this.searchID 
                                                            || (recipe.name.replace(/-/g," ").slice(0, this.searchName.length).toLowerCase() == this.searchName.toLowerCase())
    ...
    mounted() {
        axios.get('http://localhost:3500/recipe')
